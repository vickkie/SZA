...
const Menu: React.FC<MenuProps> = ({ open, setOpen, preloaded, windowWidth }) => {
  const menuRef = useRef<HTMLDivElement>(null);
  const feDisplacementMapRefs = useRefArray<SVGFEDisplacementMapElement>(4);
  const { ref: enterTimeline, setStateRef: setEnterTimeline } = useStateRef<null | gsap.core.Timeline>(null);
  ...
  return (
    <div id="menu" ref={menuRef} className={S.menu}>
      <svg className="hidden">
        <defs>
          <filter id="filter-0">
            <feTurbulence result="warp" numOctaves="6" type="fractalNoise" baseFrequency="0.2 0.025" />
            <feDisplacementMap
              scale="0"
              in2="warp"
              in="SourceGraphic"
      ...
      <Link to="/">
        <p data-filter="0" data-splitting="" onClick={onClick} className={`${S.menuItem} split-text menuItem`}>
          Home <Icon className={S.icon} />
        </p>
      </Link>
      <Link to="/gallery">
        <p data-filter="1" data-splitting="" onClick={onClick} className={`${S.menuItem} split-text menuItem`}>
          Gallery <Icon className={S.icon} />
        </p>
      </Link>
      <Link to="/tour">
        <p data-filter="2" data-splitting="" onClick={onClick} className={`${S.menuItem} split-text menuItem`}>
          Tour <Icon className={S.icon} />
        </p>
      </Link>
      <Link to="/credits">
        <p data-filter="3" data-splitting="" onClick={onClick} className={`${S.menuItem} split-text menuItem`}>
